...
def realize_f_s_p(quant_it, d=0.1, alpha=.2, beta=.1,initial = np.zeros(4),  c=2):
    elements = np.zeros((2 * c, quant_it))
    for i in range(4):
        elements[i][0] = initial[i]
    for i in range(1, quant_it):
        for j in range(2 * c - 1):
            elements[j][i] = elements[j + 1][i - 1]
        elements[2 * c - 1][i] = f.two_c_stat_solve_piecewise(d, alpha, beta, elements[c + 1][i - 1],
                                                              elements[c][i - 1],
                                                              elements[0][i - 1], c)
    return elements
