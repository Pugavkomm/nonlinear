...
for i in range (1):
    s_d = s_d + .0
    print('b = ', 1 - 2 * s_d - 2 * s_alpha)
    r = kof (0, s_d, s_alpha, 1)  # незабыть убрать с.р.
    ...
    for j in range (4):
        ...
        if j == 3:
            plt.plot (r[j].real, r[j].imag, '.', color=col, label='d  =' + str ( s_d))
